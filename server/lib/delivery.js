...
function deliveryOffer(q, goods) {
    ...
    // v14.35 — 🪤 كان هنا `reason: 'out_of_zone'` لكل حالة غير متاحة. والقاعدة
    // تفرّق بين أربعة أسباب مختلفة تماماً (`no_zones` · `no_location` ·
    // `out_of_zone` · `disabled`)، فكان المشتري يقرأ «عنوانك خارج نطاق التوصيل»
    // بينما الحقيقة أن **التاجر لم يرسم نطاقاً بعد** — فيظنّ العيب في عنوانه
    // ويعدّله مراراً بلا جدوى. نمرّر سبب القاعدة كما هو.
    if (!q.available) return { ...base, reason: q.reason || 'out_of_zone' };
