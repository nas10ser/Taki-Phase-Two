...
- **الإصدار الحالي: v14.37** (آخر إصدار في `progress.md` — الأحدث في الأعلى) · البوت `14.35.0`
...
## 🔴 ثغرتان أمنيّتان كُشفتا ١٤ سبتمبر ٢٠٢٦ — واحدة سُدّت وواحدة مفتوحة
...
**🔴 مفتوحة — تنفجر لحظة منح أول صلاحية فرعية:** `users_select_own_or_admin`
و`users_update_admin` (وسياسات ٢٧ جدولاً آخر) تعتمد `is_admin()`، وهي تُرجع
TRUE لأي أدمن حين **لا يوجد إطار `admin_*` على مكدّس النداء** — أي عند أي قراءة
أو كتابة مباشرة على الجدول من المتصفّح. قِيس فعلياً: أدمن فرعي بصلاحية
«الرئيسية» وحدها **قرأ جوّالات وبُرُد كل العملاء، وأوقف متجرين** بكتابة مباشرة
تتجاوز كل الدوال الإدارية. لا خطر اليوم (ناصر وحده أدمن، وهو أدمن أعلى)، لكن
**لا تُمنح صلاحية فرعية لأحد قبل إصلاح هذا.**
وستّ من تسع صلاحيات `action_*` لا تحرس شيئاً في أي مكان — مجرّد مربّعات اختيار.
