...
- الإنتاج: **`https://www.takisa.net`** (النطاق الرئيسي — الجذر `takisa.net` يحوّل إليه بـ308؛ و`taki-test-eight.vercel.app` يبقى يعمل)
...
## 🎯 الناقص (بالأولوية)
1. 🔴 **البريد** — المُرسِل عنوان Resend المشترك ولا يصل إلا لناصر ⇒ **لا يستطيع أحد التسجيل**. النطاق صار جاهزاً، فالمتبقّي توثيقه في Resend (SPF + DKIM + DMARC على Cloudflare) وتبديل `MAIL_FROM` إلى `no-reply@takisa.net`. **هذا أعلى بند أثراً الآن — بدونه لا نمو.**
2. 🔴 **نسخة احتياطية خارج الخادم** — النسخ اليومية على نفس الجهاز؛ فقدُه يفقدها.
3. 🟠 **١٨٩ دالة موجودة على الخادم فقط** — النسخة اليومية تغطّيها، لكن المستودع وحده لا يُعيد البناء (والمستودع عام فلا يصلح لحمل المخطط بلا فحص أسرار).
4. 🟠 **CAPTCHA + منع كلمات المرور المسرَّبة** — إعدادان في خدمة الحسابات، لم يُفعَّلا بعد.
5. ⚪ **جدولة الفحص الأمني** — `.github/workflows/security-probe.yml` مكتوب محلياً ولا يُرفع: مفتاح GitHub ينقصه صلاحية `workflow`. يُشغَّل يدوياً: `cd ~/Desktop/TAKI && bash scripts/security-probe.sh`.

**أُنجز:** ✅ النطاق (`www.takisa.net` + `api.takisa.net`، الجذر يحوّل بـ308) · ✅ مراقبة التوقّف كل ١٠ دقائق من خوادم GitHub · ✅ الفحص الأمني الحيّ ٣٦/٣٦ · ✅ سطح انتحال البوت = صفر (v13.84).
