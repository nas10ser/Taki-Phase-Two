...
/**
 * شعار المتجر — مصدر واحد للحقيقة (v13.71).
 *
 * الصف في القاعدة يسمّي العمود `avatar_url`، بينما بعض الشاشات كانت تقرأ
 * `profile.avatar` (اسم لا يُنتجه أي مُحوِّل في المستودعات) — فكان الشعار
 * **لا يظهر أبداً** في صفحة تفاصيل العرض مهما رفع التاجر صورته، ويسقط دائماً
 * إلى الإيموجي 🏪. الدالة تقبل الشكلين وتُرجع فراغاً حين لا يوجد شعار، فيقرّر
 * المكوّن ماذا يعرض بدله (حرف الاسم أو الإيموجي) بدل `<img src="">` مكسور.
 */
export const storeAvatar = (profile: any): string =>
    String(profile?.avatar_url || profile?.avatar || '');
