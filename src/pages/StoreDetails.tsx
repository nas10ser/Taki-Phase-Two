...
const StoreDetails: React.FC = () => {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const { data } = await supabase.from('store_profiles')
                .select('max_branches').eq('store_id', user.id).maybeSingle();
            if (alive && Number(data?.max_branches) > 0) setOwnerMaxLocations(Number(data!.max_branches));
    ...
    const reActivateDeal = async (deal: any) => {
        ...
        if (!confirmed) return;
        // v13.17 — مواءمة مع الباقة (قصّ المواقع الزائدة) + تجديد العمر والتاريخ
        ...
        try {
            ...
            if (!ok) {
                // Renew REJECTED by the DB (almost always: the deal's old
                // location slot was deleted/changed and the location-cap
                // trigger refuses to re-activate it). v12.72: open the EDIT
                // FORM for this exact deal pre-filled (the deleted-location
        ...
        <div className="page-content" style={{ background: 'var(--body-bg)', minHeight: '100vh', direction: isRTL ? 'rtl' : 'ltr' }}>
            ...
            <div className="animate-fade-in" style={{
                                ...
                                {storeAuth.show && (
                                    <>
                                        ...
                                        <div style={cell} title={isRTL ? `${storeAuth.total} صوت على مصداقية العروض` : `${storeAuth.total} authenticity votes`}>
                                            ...
                                            <div style={label}>{storeAuth.real ? (isRTL ? 'عروض حقيقية' : 'Real offers') : (isRTL ? 'عروض شكلية' : 'Cosmetic offers')}</div>
                                        </div>
                                    </>
                                )}
                ...
                {/* v13.74 (أمر ناصر: «أخرِج المحدَّد من صندوق الاتصال والواتساب
                    وضعه أسفله»): شريط تبويبات التاجر خرج من بطاقة «نبذة عن المحل»
                    وصار شريطاً مستقلاً **تحتها**. كان داخل البطاقة فيبدو جزءاً من
                    بيانات المتجر التي يراها الزبون، وهو أدوات إدارة لصاحب المتجر
                    وحده. (v13.72 أنزله تحت أزرار التواصل، وهذه الخطوة تُخرجه من
                    الصندوق أصلاً.) */}
                {isOwner && (
                        ...
                        <div className="taki-deals-grid" style={{ display: 'grid', gap: 10 }}>
                            {pastStoreDeals.length > 0 ? pastStoreDeals.map(deal => (
                                <div key={deal.id} style={{ display: 'flex', flexDirection: 'column', opacity: 0.85 }}>
                                    ...
                                    {user?.id === store?.id && (
                                        <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                                            {/* v12.72 — نفس أبعاد أزرار النشطة + minWidth: 0 (منع فيض
                                                الشبكة) + ⏳ أثناء الحفظ؛ الاختلاف الوحيد لون التفعيل/التجديد */}
                                            <button
                                                onClick={() => {
