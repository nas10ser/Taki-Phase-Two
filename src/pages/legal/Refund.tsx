        ...
        >
            ...
            <Section n={3} title={isRTL ? '📅 إلغاء الحجز (للمشتري)' : '📅 Cancelling a booking (buyers)'}>
                <Bullets items={isRTL ? [
                    ...
                    <><strong>الطلب غير المدفوع</strong> (الدفع عند الاستلام): يحقّ لك إلغاؤه في أيّ وقت قبل انتهاء صلاحيّته من صفحة «حجوزاتي» بضغطة واحدة، دون أيّ رسوم أو غرامات — وتعود الكمّيات المحجوزة للعرض فوراً، ولا يترتّب عليه أيّ مبلغ لأنّه لا دفع تمّ أصلاً.</>,
                    <><strong>الطلب المدفوع إلكترونياً لا يُلغى بضغطة واحدة.</strong> المال في حساب التاجر لا عند TAKI، فالإلغاء وحده يتركك بلا بضاعة ولا مال. الزرّ في «حجوزاتي» يُرسل <strong>طلب إلغاء واسترداد</strong> يصل التاجر فوراً، ويبقى طلبك قائماً حتى يبتّ فيه.</>,
                    <>قرار الاسترداد وتنفيذه على <strong>التاجر</strong> وفق سياسته المُعلَنة في صفحة متجره وصفحة العرض. TAKI تُسجّل طلبك وتُبلّغ التاجر وتُثبت ما جرى بتاريخ ومرجع ومبلغ وتُظهره إشعاراً دائناً على فاتورتك — ولا تحتفظ بالمال ولا تردّه ولا تبتّ في الخلاف.</>,
                    ...
                    <><strong>Unpaid orders</strong> (cash on collection): you may cancel at any time before expiry from «My Bookings» in a single tap, with no fees or penalties — the reserved quantities return immediately and no amount arises, because no payment was made.</>,
                    <><strong>A card-paid order cannot be cancelled with one tap.</strong> The money sits in the merchant's account, not with TAKI, so a bare cancellation would leave you with neither goods nor money. The button in «My Bookings» sends a <strong>cancellation and refund request</strong> to the merchant, and your order stands until they decide.</>,
                    <>The refund decision and its execution rest with the <strong>merchant</strong>, under the policy published on their store page and deal page. TAKI records your request, notifies the merchant, documents what happened with a date, a reference and an amount, and shows it as a credit note on your invoice — it never holds the money, cannot return it and does not adjudicate.</>,
