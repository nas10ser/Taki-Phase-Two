...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const updateDeal = useCallback(async (deal: Deal) => {
        ...
        try {
            ...
            // v13.16 — «needs M»: عدد المواقع التي يغطيها هذا العرض فعلاً (الأساسي
            // + فروع «انشر في عدة مواقع»). يجعل رسالة تخفيض الباقة مفهومة: عرضك
            // منشور على ٣ فروع وباقتك صارت موقعاً واحداً.
            const needMatch = msg.match(/needs\s+(\d+)/i);
            const needN = needMatch ? Number(needMatch[1]) : 0;
            const capMsgAr = (needN > 1 && capN > 0 && needN > capN)
                ? `⚠️ هذا العرض منشور في ${needN} مواقع، وباقتك الحالية تسمح بـ${capN === 1 ? 'موقع واحد فقط' : `${capN} مواقع`}.\n\nافتح «تعديل» على العرض وأزل المواقع الزائدة من قسم «انشر نفس العرض في عدة مواقع» حتى يصبح العدد ${capN === 1 ? 'موقعاً واحداً' : `${capN} أو أقل`}، ثم احفظ. وللمزيد من المواقع رقِّ باقتك من «الاشتراك».`
                : capN === 1
