...
if ! curl -fsS --max-time 30 "$SITE/" -o "$TMP/index.html"; then
    # قبل اتّهام الموقع: هل العطل في **جهاز الفحص**؟ ساعةٌ خاطئة تجعل شهادة
    # سليمة تبدو «غير صالحة بعد»، فيُعلن الفحص سقوط الموقع وهو حيّ. حدث فعلاً
    # (٣١ أغسطس ٢٠٢٦: ماك ناصر متأخّر ٣ أيام ⇒ شهادة Vercel المجدَّدة رُفضت).
    # نقرأ الوقت الحقيقي من ترويسة Date عبر HTTP عادي — بلا شهادات إطلاقاً.
    remote=$(curl -sI --max-time 15 http://example.com | tr -d '\r' | grep -i '^date:' | cut -d' ' -f2-)
    if [ -n "$remote" ]; then
        r_ep=$(date -u -jf '%a, %d %b %Y %H:%M:%S GMT' "$remote" '+%s' 2>/dev/null \
               || date -u -d "$remote" '+%s' 2>/dev/null)
        l_ep=$(date -u '+%s')
        if [ -n "$r_ep" ]; then
            skew=$(( r_ep - l_ep )); [ "$skew" -lt 0 ] && skew=$(( -skew ))
            if [ "$skew" -gt 300 ]; then
                bad "ساعة **جهاز الفحص** خاطئة بفارق $(( skew / 3600 )) ساعة — والشهادات تُرفض بسببها. الموقع غالباً سليم. اضبط الوقت تلقائياً ثم أعد الفحص."
                exit 1
            fi
        fi
    fi
